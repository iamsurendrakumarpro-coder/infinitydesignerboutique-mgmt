"""utils/storage_provider.py - S3 storage helpers."""
...
def _load_boto3():
    try:
        ...
        raise RuntimeError("boto3 is not installed. Install boto3 to use AWS S3 storage") from exc
...
def upload_bytes(
    ...
    """
    Upload raw bytes to S3 storage.
    ...
    try:
        ...
        log.error("Storage upload failed | provider=s3 | path=%s | error=%s", storage_path, exc)
...
def delete_object(storage_path: str) -> tuple[bool, str]:
    ...
    try:
        ...
        log.warning("Storage delete failed | provider=s3 | path=%s | error=%s", storage_path, exc)
        return False, str(exc)


def generate_download_url(storage_path: str, expiration_minutes: int = 60) -> str:
    """Generate a time-limited download URL from S3."""
    client = _get_s3_client()
