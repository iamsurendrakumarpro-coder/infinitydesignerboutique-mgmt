    ...
    for staff in weekly_staff:
        ...
        try:
            ...
            if existing_docs:
                ...
                doc.update({
                    "staff_name": breakdown.get("staff_name", ""),
                    "full_name": breakdown.get("full_name", ""),
                    "designation": breakdown.get("designation", ""),
                    "staff_status": breakdown.get("staff_status", "active"),
...
def get_settlement(settlement_id: str) -> dict | None:
    ...
    return _enrich_settlement(_sanitise(doc.to_dict()))
...
def get_settlements(filters: dict | None = None) -> list[dict]:
    ...
    for s in settlements:
        ...
        if key not in deduped or s.get("created_at","") > deduped[key].get("created_at",""):
            deduped[key] = s
    result = [_enrich_settlement(s) for s in deduped.values()]
...
def get_settlements_for_user(user_id: str) -> list[dict]:
    ...
    results = [_enrich_settlement(_sanitise(d.to_dict())) for d in docs]
...
def _enrich_settlement(data: dict) -> dict:
    """Backfill current staff metadata for older settlement documents."""
    ...
    staff = get_staff(user_id)
    if not staff:
        out.setdefault("staff_status", out.get("status", "inactive"))
        return out

    out["staff_name"] = out.get("staff_name") or staff.get("full_name") or out.get("full_name", "")
    out["full_name"] = out.get("full_name") or staff.get("full_name", "")
    out["designation"] = out.get("designation") or staff.get("designation", "")
    out["staff_status"] = staff.get("status", out.get("staff_status", "inactive"))
    return out
