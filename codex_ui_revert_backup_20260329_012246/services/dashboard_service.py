...
def get_attendance_summary(start_date: date, end_date: date) -> dict:
    ...
    standard_hours = float(working_config.get("standard_hours_per_day", 8) or 8)
    ...
    for staff in active_staff:
        ...
        expected_minutes = days * standard_minutes
        variance_minutes = mins - expected_minutes
        ...
        staff_summaries.append({
            ...
            "expected_duration": minutes_to_hhmm(expected_minutes),
            "variance_minutes": variance_minutes,
            "variance_duration": minutes_to_hhmm(abs(variance_minutes)) if variance_minutes else "0h 0m",
            "average_minutes_per_day": round(mins / days, 2) if days else 0,
        })

        total_days_present += days
        total_minutes += mins

    total_expected_minutes = total_days_present * standard_minutes
    attendance_rate = 0.0
    if active_staff and period_days:
        attendance_rate = round((total_days_present / (len(active_staff) * period_days)) * 100, 1)
    ...
    summary = {
        ...
        "standard_minutes_per_day": standard_minutes,
        "total_days_present": total_days_present,
        "total_minutes": total_minutes,
        "total_duration": minutes_to_hhmm(total_minutes),
        "total_expected_minutes": total_expected_minutes,
        "total_expected_duration": minutes_to_hhmm(total_expected_minutes),
        "average_minutes_per_staff": round(total_minutes / len(active_staff), 2) if active_staff else 0,
        "average_minutes_per_present_day": round(total_minutes / total_days_present, 2) if total_days_present else 0,
